# Train a 1D CNN on sequences_rul.npz — same raw input as the LSTM, to
# test whether the LSTM's failure was architecture specific or a raw
# sequence issue in general. Same GroupKFold split and preprocessing
...
def main():
    ...
    X = np.clip(X, lo, hi)
    X = X[:, :, ::10, :]  # downsample 1000 -> 100 points per cycle
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X_norm, y_norm, groups)):
        ...
        if fold == 0:
            print(f"Loss history (fold 1, last 5): {history.history['loss'][-5:]}")
    ...
    print(f"\nCNN             MAE: {np.mean(maes):7.2f} +/- {np.std(maes):5.2f}"
    ...
    out_path = Path(args.sequences_npz).parent / "cnn_rul_results.csv"
